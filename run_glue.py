...
from auto_model import (
    SORSAAutoConfig,
    SORSAAutoModelForSequenceClassification as AutoModelForSequenceClassification,
)
...
def parse_args():
    ...
    #############################
    parser.add_argument(
        "--reduced_rank", type=float, default=32, help="rank of low rank adapters"
    )
    ...
    parser.add_argument(
        "--qlora", action="store_true", help="use qlora to init low rank adapters"
    )
    parser.add_argument(
        "--loftq", action="store_true", help="use loftq to init low rank adapters"
    )
    parser.add_argument(
        "--decomposed_pretrained_ckpt_path",
        type=str,
        help="path of pre-decomposed model",
    )
    parser.add_argument(
        "--quant_embedding", action="store_true", help="whether quantize embeddings"
    )
    parser.add_argument(
        "--quant_method", type=str, default="uniform", help="quantization method used"
    )
    parser.add_argument(
        "--num_iter", type=int, default="5", help="The number of iterations"
    )
...
def main():
    ...
    for name, param in model.named_parameters():
        if "sorsa_" in name or name in additions:
            param.requires_grad = True
            train_params.append(param)
            trainable += param.numel()
        else:
            param.requires_grad = False
    print(f"Trainable Parameters: {trainable / 1000**2}M")
    #####################################
    #                                   #
    #              Model                #
    #                                   #
    ...
    if args.pad_to_max_length:
        ...
        data_collator = DataCollatorWithPadding(
            tokenizer, pad_to_multiple_of=(8 if args.use_fp16 else None)
        )
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (
                ...
                scaler.step(args.gamma * calc_ortho(model))
                # optimizer.step()
            ...
            if (
                completed_steps % 500 == 0
                and step % args.gradient_accumulation_steps == 0
            ):
                print(f"The current loss is {loss}")

            if (
                completed_steps % (5000 * 32 / total_batch_size) == 1
                and step % args.gradient_accumulation_steps == 0
            ):
                model.eval()
                model.merge_sorsa(True)
                samples_seen = 0
                ...
                for k, v in eval_metric.items():
                    writer.add_scalar(
                        f"eval/{args.output_dir}/{k}", v, global_step=completed_steps
                    )
                print(
                    f"seed {args.seed} learning rate {args.learning_rate} "
                    + f"epoch {epoch}: {eval_metric}"
                )
                performace_dict[completed_steps] = eval_metric[
                    task_to_metrics[args.task_name]
                ]

                torch.save(
                    model.state_dict(), args.output_dir + f"/{completed_steps}.bin"
                )
    ...
    if not eval:
        ...
        max_keys = [
            k for k, v in performace_dict.items() if v == best_performance
