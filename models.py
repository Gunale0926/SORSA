...
from transformers import LlamaConfig, GemmaConfig
from tqdm import tqdm


class BaseSORSAModel:
    def __init__(self, target, rank, sorsa_dropout):
        self.target = target
        self.replaced = []
        self.rank = rank
...
class SORSARobertaForSequenceClassification(
    ...
    def __init__(self, config):
        ...
        BaseSORSAModel.__init__(self, config.target, config.rank, config.sorsa_dropout)
        self._sorsa_replace()
...
class SORSARobertaConfig(RobertaConfig):
    ...
    def __init__(self, rank=None, sorsa_dropout=0.1, target=None, **kwargs):
