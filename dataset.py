...
def extract_answer(completion):
    """
    Extract answer from MetaMathQA format completion.
    """
...
class GSM8KDataset(Dataset):
    # This dataset is used for test ONLY!
    def __init__(self, file_path, tokenizer, max_length=512):
        self.data = pd.read_parquet(file_path).to_dict(orient="records")
    ...
    def collate_fn(self, batch):
        ...
        query = torch.nn.utils.rnn.pad_sequence(
            query, batch_first=True, padding_value=self.tokenizer.pad_token_id
        )
...
class MATHDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        answer = remove_boxed(util.last_boxed_only_string(item["output"]))
        text = f"Below is an instruction that describes a task. Write a response that appropriately completes the request.\n\n### Instruction:\n{question}\n\n### Response: Let's think step by step."
        query = self.tokenizer.encode(
            text=text,
            truncation=False,
